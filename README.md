...
services:
  couchdb:
    ...
    - 5984:5984
```
执行`docker-compose -f couchdb.yaml up -d`
使用http://ip:5984/_utils访问couchdb的后台管理系统。
...
转账(至少要转两笔才能确认，可以修改txpool.py的SIZE属性来调整区块大小)。注意：只有当前有这个地址（即有这个私钥）才能作为from转账给其他地址。
```bash
...
这里因为重复转了两笔账，使用了同一个UTXO，所以第二笔会失败，由于`1LYHea8NjTxaYboXJbR7LemvUZjyQc839r`为被奖励地址，所以获得了1000得挖矿奖励所以余额为:1000-100+1000=1900。
# BlockchainPrototype
